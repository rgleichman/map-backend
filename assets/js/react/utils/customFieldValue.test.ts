...
describe("formatCustomFieldValue", () => {
  it("uses select option labels", () => {
    ...
    expect(formatCustomFieldValue(field, "jazz")).toBe("Jazz")
  })
})

describe("searchableCustomFieldText", () => {
  it("returns null for blob refs", () => {
    expect(searchableCustomFieldText(drawingField, { ref: 1 })).toBeNull()
  })

  it("returns formatted text for primitives", () => {
    const field = { key: "artist", label: "Artist", type: "text" as const }
    expect(searchableCustomFieldText(field, "Beatles")).toBe("Beatles")
  })
})
