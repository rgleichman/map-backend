...
- Use `mix precommit` alias when you are done with all changes and fix any pending issues. The project has a git pre-commit hook that runs it; contributors run `./scripts/install-git-hooks` once to enable it.
...
- **daisyUI is configured and available** (themes in app.css; see core_components.ex). Use daisyUI theme and component classes where they fit the design; use Tailwind utilities for custom styling.
...
| Exit immersive editor | **Done** | Blob `FieldEditorModal` (mobile and desktop; Escape also works) |
